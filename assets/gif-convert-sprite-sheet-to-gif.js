window.bridges['animate-bitmap-sprites'] = function () {
  var input = { data: null, preview: null };
  ...
  var bridge = function () {
    ...
    var inputCtxData = input.data.getContext('2d');
    var outputCtxData = output.data.getContext('2d');
    var inputCtxPreview = input.preview.getContext('2d');
    var outputCtxPreview = output.preview.getContext('2d');
    output.data.width = opts.spriteWidth;
    output.data.height = opts.spriteHeight;
    var inputFit = best_image_fit(input.data.width, input.data.height, input.preview.width, input.preview.height);
    inputCtxPreview.clearRect(0, 0, input.preview.width, input.preview.height);
    fillTransparencyEffect(input.preview);
    inputCtxPreview.drawImage(input.data, inputFit.offsetX, inputFit.offsetY, inputFit.width, inputFit.height);
    var inputImage = inputCtxPreview.getImageData(0, 0, input.preview.width, input.preview.height);
    var outputFit = best_image_fit(
      output.data.width * opts.zoom,
      output.data.height * opts.zoom,
      output.preview.width,
      output.preview.height
    );
    outputCtxPreview.clearRect(0, 0, output.preview.width, output.preview.height);
    fillTransparencyEffect(output.preview);
    var outputImage = outputCtxPreview.getImageData(0, 0, output.preview.width, output.preview.height);
    if (opts.orientation == 'horizontal') {
      var totalFrames = Math.round((input.data.width - opts.startPointX) / opts.spriteWidth);
    } else {
      var totalFrames = Math.round((input.data.height - opts.startPointY) / opts.spriteHeight);
    }
    gifFrames = [];
    var framesToDraw = [];
    var allFrames = [];
    for (var i = 0; i < totalFrames; i++) {
      ...
      if (opts.skipSprite.indexOf(i + 1) < 0) {
        framesToDraw.push(frame);
        var sprite = getSprite(inputCtxData, opts, frame);
        gifFrames.push(sprite);
      }
      allFrames.push(frame);
    }
    if (opts.direction == 'right-to-left' || opts.direction == 'bottom-to-top') {
      framesToDraw.reverse();
      gifFrames.reverse();
    }
    if (framesToDraw.length == 0) {
      error('No Frames to Animate', "All sprites were skipped and there's nothing to animate.");
      return;
    }
    var count = 0;
    ...
    function startAnimation() {
      drawFrame();
      if (opts.play) {
        timer = setInterval(drawFrame, opts.duration);
      }
      return timer;
    }
    function drawFrame() {
      inputCtxPreview.putImageData(inputImage, 0, 0);
      if (opts.play) {
        drawStrokeRectangle(inputCtxPreview, inputFit, opts, framesToDraw[count]);
      } else {
        for (var i = 0; i < totalFrames; i++) {
          enumerateFrames(inputCtxPreview, inputFit, opts, allFrames[i]);
          if (opts.skipSprite.indexOf(i + 1) < 0) {
            drawStrokeRectangle(inputCtxPreview, inputFit, opts, allFrames[i]);
          }
        }
      }
      outputCtxData.putImageData(gifFrames[count], 0, 0);
      changeImageBackgound(outputCtxData, opts);
      tool.respond();
      outputCtxPreview.putImageData(outputImage, 0, 0);
      outputCtxPreview.drawImage(output.data, outputFit.offsetX, outputFit.offsetY, outputFit.width, outputFit.height);
      count = count >= framesToDraw.length - 1 ? 0 : count + 1;
      if (!opts.play) clearInterval(timer);
    }
  };
  function changeImageBackgound(ctx, opts) {
    if (opts.inputBackground) {
      var pixels = ctx.getImageData(0, 0, input.data.width, input.data.height);
      inputBackground = colorToRGBA(opts.inputBackground);
      outputBackground = colorToRGBA(opts.outputBackground);
      for (var i = 0; i < pixels.data.length; i += 4) {
        var color = { r: pixels.data[i + 0], g: pixels.data[i + 1], b: pixels.data[i + 2], a: pixels.data[i + 3] };
        var delta = rgbaDifference(color, inputBackground);
        if (delta <= opts.threshold) {
          pixels.data[i + 0] = outputBackground.r;
          pixels.data[i + 1] = outputBackground.g;
          pixels.data[i + 2] = outputBackground.b;
          pixels.data[i + 3] = outputBackground.a;
        }
      }
      ctx.putImageData(pixels, 0, 0);
    }
  }
  function getSprite(ctx, opts, curFrame) {
    var x = opts.startPointX + curFrame.x * opts.spriteWidth;
    var y = opts.startPointY + curFrame.y * opts.spriteHeight;
    var width = opts.spriteWidth;
    var height = opts.spriteHeight;
    return ctx.getImageData(x, y, width, height);
  }
  function drawStrokeRectangle(ctx, fit, opts, curFrame) {
    var x = fit.offsetX + opts.startPointX / fit.scale + (opts.spriteWidth / fit.scale) * curFrame.x;
    var y = fit.offsetY + opts.startPointY / fit.scale + (opts.spriteHeight / fit.scale) * curFrame.y;
    var width = opts.spriteWidth / fit.scale;
    var height = opts.spriteHeight / fit.scale;
    ctx.strokeStyle = 'green';
    ctx.lineWidth = 1;
    ctx.strokeRect(x, y, width, height);
  }
  function enumerateFrames(ctx, fit, opts, curFrame) {
    var spriteWidth = opts.spriteWidth / fit.scale;
    var spriteHeight = opts.spriteHeight / fit.scale;
    var startPointX = opts.startPointX / fit.scale;
    var startPointY = opts.startPointY / fit.scale;
    var padding = spriteWidth / 15;
    var x = fit.offsetX + startPointX + spriteWidth * curFrame.x + padding;
    var y = fit.offsetY + startPointY + spriteHeight * (curFrame.y + 1) - padding;
    if (opts.orientation == 'horizontal') {
      var text = (curFrame.x + 1).toString();
    } else {
      var text = (curFrame.y + 1).toString();
    }
    ctx.font = spriteWidth / 5 + ' Arial';
  ...
  function parseOptions(tool, error) {
    ...
    var duration = options['duration'].trim() || '200';
    if (!/^[+-]?\d+$/.test(duration)) {
      error('Invalid Playback Speed', 'Playback speed contains non-digits.');
      return false;
    }
    duration = parseInt(duration);
    if (duration < 1) {
      error('Invalid Playback Speed', 'Playback speed must be greater than zero.');
      return false;
    }
    var spriteWidth = options['sprite-width'].trim();
    if (!/^[+-]?\d+$/.test(spriteWidth)) {
      error('Invalid Sprite Width', 'Sprite width contains non-digits.');
      return false;
    }
    spriteWidth = parseInt(spriteWidth);
    if (spriteWidth < 1) {
      error('Invalid Sprite Width', 'Sprite width must be greater than zero.');
      return false;
    }
    var spriteHeight = options['sprite-height'].trim();
    if (!/^[+-]?\d+$/.test(spriteHeight)) {
      error('Invalid Sprite Height', 'Sprite height contains non-digits.');
      return false;
    }
    spriteHeight = parseInt(spriteHeight);
    if (spriteHeight < 1) {
      error('Invalid Sprite Height', 'Sprite height must be greater than zero.');
      return false;
    }
    var startPointX = options['start-point-x'].trim() || 0;
    if (!/^[+-]?\d+$/.test(startPointX)) {
      error('Invalid Start Point X', 'Start point X contains non-digits.');
      return false;
    }
    startPointX = parseInt(startPointX);
    if (startPointX < 0) {
      error('Invalid Start Point X', 'Start point X must be a positive number.');
      return false;
    }
    var startPointY = options['start-point-y'].trim() || 0;
    if (!/^[+-]?\d+$/.test(startPointY)) {
      error('Invalid Start Point Y', 'Start point Y contains non-digits.');
      return false;
    }
    startPointY = parseInt(startPointY);
    if (startPointY < 0) {
      error('Invalid Start Point Y', 'Start point Y must be a positive number.');
      return false;
    }
    var zoom = options['zoom'].replace(/\s*%\s*$/, '');
    if (!/^[+-]?\d*\.?\d+$/.test(zoom)) {
      error('Invalid Zoom', 'Zoom contains non-digits.');
      return false;
    }
    zoom = parseFloat(zoom / 100);
    if (zoom <= 0) {
      error('Invalid Zoom', 'Zoom must be a positive number.');
      return false;
    }
    var range = parseRange(options['skip-sprite']);
    if (!range.success) {
      error('Invalid Skip Sprite Range', range.error);
      return false;
    }
    var skipSprite = range.numbers;
    var inputBackground = options['input-background'].trim();
    if (/^[0-9a-f]+$/i.test(inputBackground)) {
      inputBackground = '#' + inputBackground;
    }
    if (inputBackground && !isColorValid(inputBackground)) {
      error('Invalid Input Background Color', 'Input background color is not valid.');
      return false;
    }
    var outputBackground = options['output-background'].trim() || 'transparent';
    if (/^[0-9a-f]+$/i.test(outputBackground)) {
      outputBackground = '#' + outputBackground;
    }
    if (outputBackground && !isColorValid(outputBackground)) {
      error('Invalid Output Background Color', 'Output background color is not valid.');
      return false;
    }
    var threshold = options['threshold'].replace(/\s*%\s*$/, '');
    if (inputBackground) {
      if (!/^[+-]?\d*\.?\d+$/.test(threshold)) {
        error('Invalid Threshold', 'Threshold contains non-digits.');
        return false;
      }
      threshold = parseFloat(threshold);
      if (threshold < 0 || threshold > 100) {
        error('Invalid Threshold', 'Threshold must be between 0 and 100.');
        return false;
      }
    }
  ...
  function gifContainTranspPixels(gifFrames) {
    for (var i = 0; i < gifFrames.length; i++) {
      var pixels = gifFrames[i];
  ...
  function isTransparent(color) {
    return colorToRGBA(color).a == 0;
  }
  function makeSemiTransparentPixelsTransparent(ctx, canvas) {
  ...
  function downloader(cb) {
    ...
    var spriteWidth = Math.ceil(gifFrames[0].width * opts.zoom);
    var spriteHeight = Math.ceil(gifFrames[0].height * opts.zoom);
    var inputBackground = opts.inputBackground;
    var outputBackground = opts.outputBackground;
    var solidCanvas = document.createElement('canvas');
    var solidCanvasCtx = solidCanvas.getContext('2d');
    solidCanvas.width = spriteWidth;
    solidCanvas.height = spriteHeight;
    var frameCanvas = document.createElement('canvas');
    var frameCanvasCtx = frameCanvas.getContext('2d');
    frameCanvas.width = gifFrames[0].width;
    frameCanvas.height = gifFrames[0].height;
    if (
      !gifContainTranspPixels(gifFrames) &&
      inputBackground &&
      !isTransparent(inputBackground) &&
      isTransparent(outputBackground)
    ) {
      var transparentColor = 'cyan';
    } else if (
      gifContainTranspPixels(gifFrames) &&
      !(isTransparent(inputBackground) && !isTransparent(outputBackground))
    ) {
      var transparentColor = 'cyan';
    } else {
      var transparentColor = false;
    }
    solidCanvasCtx.fillStyle = transparentColor;
    var encoder = new GIFEncoder();
    encoder.setSize(spriteWidth, spriteHeight);
    encoder.setDelay(opts.duration);
    encoder.setRepeat(0);
    encoder.setQuality(6);
    encoder.start();
    for (var i = 0; i < gifFrames.length; i++) {
      if (transparentColor) {
        encoder.setTransparent(colorToRGB(transparentColor).toInteger());
        solidCanvasCtx.fillRect(0, 0, spriteWidth, spriteHeight);
      }
      frameCanvasCtx.putImageData(gifFrames[i], 0, 0);
      changeImageBackgound(frameCanvasCtx, opts);
      var zoomFrameCanvas = document.createElement('canvas');
      var zoomFrameCanvasCtx = zoomFrameCanvas.getContext('2d');
      zoomFrameCanvas.width = spriteWidth;
      zoomFrameCanvas.height = spriteHeight;
      zoomFrameCanvasCtx.imageSmoothingEnabled = false;
      zoomFrameCanvasCtx.webkitImageSmoothingEnabled = false;
      zoomFrameCanvasCtx.mozImageSmoothingEnabled = false;
      zoomFrameCanvasCtx.msImageSmoothingEnabled = false;
      zoomFrameCanvasCtx.oImageSmoothingEnabled = false;
      zoomFrameCanvasCtx.drawImage(frameCanvas, 0, 0, spriteWidth, spriteHeight);
      makeSemiTransparentPixelsTransparent(zoomFrameCanvasCtx, zoomFrameCanvas);
      solidCanvasCtx.drawImage(zoomFrameCanvas, 0, 0, spriteWidth, spriteHeight);
  ...
  return {
    ...
    config: {
      type: 'image',
      input: { import: 'base64', noClipboard: true, download: getExtension, image: true },
  ...
  function spectrum(element, o) {
    ...
    function updateHelperLocations() {
      ...
      if (allowEmpty && isEmpty) {
        ...
        slideHelper.css({ left: slideX - slideHelperWidth / 2 + 'px' });
      }
    }
...
function parseRange(range) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (!/^\d+$/.test(clearedItem) && !/^\d+ *- *\d+$/.test(clearedItem)) {
      return { success: false, error: 'Range numbers must be comma or dash separated.' };
    }
...
GIFEncoder = function () {
  ...
  var addFrame = (exports.addFrame = function addFrame(im, is_imageData, strictTransp, width_, height_, x_, y_) {
    var x = 0;
    var y = 0;
  ...
  var analyzePixels = function analyzePixels(strictTransp) {
    ...
    if (transparent != null) {
      ...
      for (var i = 0; i < colorTab.length; i += 3) {
        ...
        if (colorTab[i] == r && colorTab[i + 1] == g && colorTab[i + 2] == b) trans_indices.push(index);
      }
      for (var i = 0; i < indexedPixels.length; i++)
        if (trans_indices.indexOf(indexedPixels[i]) >= 0) indexedPixels[i] = transIndex;
    }
  };
  var findClosestInRange = function findClosestInRange(c) {
    ...
    for (var i = 0; i < len; ) {
      ...
      if (usedEntry[index] && drabs < 15 && dgabs < 15 && dbabs < 15 && d < dmin) {
        dmin = d;
        minpos = index;
      }
      i++;
